// My PS Suggestion : NOT TO BE USED AROUND DATABASE - The library does not have a security check right now, may be removed later
...
export const hoursAgo = (hours: number) =>
  dayjs().subtract(hours, 'hours').startOf('hours');

export const daysAgo = (days: number) =>
  dayjs().subtract(days, 'days').startOf('day');

export const weeksAgo = (weeks: number) =>
  dayjs().subtract(weeks, 'week').startOf('week');

export const monthsAgo = (months: number) =>
  dayjs().subtract(months, 'month').startOf('month');
