...
import * as schema from '@repo/api/database/schemas';
import { merge } from '@repo/api/utils/merge';

import { UserModel } from '@repo/api/database/models/user';
import type { OrgDatabase } from '@repo/api/database/type';
import {
  mapAdapterUserToOrgUser,
  mapAuthenticatorQueryResutlToAdapterAuthenticator,
  mapOrgUserToAdapterUser,
  partialMapAdapterUserToOrgUser,
} from './utils';
...
export function OrgNextAuthDbAdapter(serverDB: OrgDatabase): Adapter {
  return {
    ...
    async createUser(user): Promise<AdapterUser> {
      ...
      if (existingUser) {
        const adapterUser = mapOrgUserToAdapterUser(existingUser);
        return adapterUser;
      }
    ...
    async getSessionAndUser(sessionToken): Promise<{
      ...
      const adapterUser = mapOrgUserToAdapterUser(result.user);
    ...
    async getUser(id): Promise<AdapterUser | null> {
      ...
      return mapOrgUserToAdapterUser(OrgUser);
    },
    ...
    async getUserByAccount(account): Promise<AdapterUser | null> {
      ...
      return result?.users ? mapOrgUserToAdapterUser(result.users) : null;
    },
    ...
    async getUserByEmail(email): Promise<AdapterUser | null> {
      const OrgUser =
        email && typeof email === 'string' && email.trim()
          ? await UserModel.findByEmail(serverDB, email)
          : undefined;
      return OrgUser ? mapOrgUserToAdapterUser(OrgUser) : null;
    },
