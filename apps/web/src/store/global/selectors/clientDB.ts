...
const displayMigrationStatus = (s: GlobalState) => {
  ...
  return (
    sql
      ...
      .map((item, index) => {
        const recordInTable = tableRecords.find(
          (record) => record.hash === item.hash
        );
        return {
          ...
          id: item.hash,
          index: index + 1,
          migratedAt: recordInTable
            ? new Date(recordInTable.created_at)
            : undefined,
          sql: item.sql,
...
const errorMigrations = (s: GlobalState) => {
  ...
  return sql.filter(
    (item) => !tableRecords.some((record) => record.hash === item.hash)
  );
};
