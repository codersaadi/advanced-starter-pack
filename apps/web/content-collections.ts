...
const isMultiLingual: boolean = process.env.IS_MULTILINGUAL_ENABLED === "true";
const withMultiLingualPath = (path: string, locale: string | undefined) =>
  isMultiLingual && locale ? `/${locale}/${path}` : path;
...
const BlogCollection = defineCollection({
  ...
  schema: z.object({
    ...
    title: z.string(),
    date: z.coerce.date(), // <<<< CORRECTED HERE: Use z.coerce.date()
    ...
    content: z.string(), // Add this to access the raw content string
  }),
  ...
  transform: async (document, utils) => {
    ...
    return {
      ...document,
      _id: withMultiLingualPath(slugWithoutLocale, locale), // Unique ID including locale,
      _raw,
      body: {
        code: mdxBody,
        frontmatter: {},
      ...
      permalink: withMultiLingualPath(`/blog/${slugWithoutLocale}`, locale), // Locale-specific permalink
    };
  },
});
